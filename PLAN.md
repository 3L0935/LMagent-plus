...
- **2026-04-02** — Fix daemon : `LocalBackendManager` toujours instancié au boot (JIT, coût nul) — supprime l'erreur "local backend not enabled" après `/setup` wizard. Wizard confirm : Enter = apply (était cancel). Wizard simplifié 7→5 steps (steps clés API supprimés).
