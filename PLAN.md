...
- **2026-04-05** — Phase 5.6 complete. Multi-agent UX + daemon fixes: Header with version + clock, persona picker with D key to toggle tabs, F2 to open picker, status tags ("▶ running"/"○ open"), messages go to active tab, hint bar with "▌ thinking…" indicator, JIT idle countdown now starts after response completes (not during load).
